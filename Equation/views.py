...
def start(request):
    ...
    if request.method == 'POST':
        ...
        if a == 0:
            ...
            return render(request, 'start.html', {'a': a, 'b': b, 'c': c, 'x': x})
        else:
            ...
            if D > 0:
                ...
                return render(request, 'start.html', {'a': a, 'b': b, 'c': c, 'D': D, 'x1': x1, 'x2': x2})
            elif D == 0:
                x3 = (-b + math.sqrt(D)) / (2 * a)
                return render(request, 'start.html', {'a': a, 'b': b, 'c': c, 'D': D, 'x3': x3})
            else:
                x4 = 'Уравнение не имеет решений'
                return render(request, 'start.html', {'a': a, 'b': b, 'c': c, 'D': D, 'x4': x4})
    else:
